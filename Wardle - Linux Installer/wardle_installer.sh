...
if [ ! -f "$INSTALL_DIR/logic/dictionary.txt" ]; then
    ...
    curl -s -L https://raw.githubusercontent.com/dwyl/english-words/master/words_alpha.txt | tr -d '\r' | grep -E '^.{5}$' | tr '[:lower:]' '[:upper:]' > "$INSTALL_DIR/logic/dictionary.txt"
fi
...
while read -r line; do
    ...
    case "\$action" in
        "guess")
            word=\$(echo "\$line" | jq -r '.word' | tr '[:lower:]' '[:upper:]')
