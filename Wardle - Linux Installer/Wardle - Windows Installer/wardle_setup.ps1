...
try {
    ...
    [System.IO.File]::WriteAllLines("$INSTALL_DIR\logic\dictionary.txt", $filtered_words)
} catch {
    ...
    for line in sys.stdin:
        ...
        if req['action'] == 'guess':
            ...
            if os.path.exists(os.path.join(PIPE_DIR, 'rematch_p1')) and os.path.exists(os.path.join(PIPE_DIR, 'rematch_p2')):
             ...
             enemy_id = 2 if my_id == 1 else 1
             winner_name = "BLUE" if my_id == 1 else "RED"
             with open(os.path.join(PIPE_DIR, 'scores.json'), 'w') as f: f.write(json.dumps({str(my_id): 2, str(enemy_id): 0}))
             try:
                 target = open(os.path.join(PIPE_DIR, 'current_word'), 'r').read().strip()
                 conn = sqlite3.connect(DB_PATH)
                 c = conn.cursor()
                 s1, s2 = (2, 0) if my_id == 1 else (0, 2)
                 c.execute("INSERT INTO match_history (winner, target_word, score_blue, score_red) VALUES (?, ?, ?, ?)", (winner_name, target, s1, s2))
                 conn.commit(); conn.close()
             except: pass
             hist = get_full_history()
             send_json({"type": "ask_name"})
             log_broadcast({"type": "wait_for_leaderboard_proxy", "winner_id": my_id, "history": hist, "msg": "Opponent disconnected. You win!"})
